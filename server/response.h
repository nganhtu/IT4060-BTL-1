...
int getRG_response(char **res, int &n, char mess[], int mess_len, users &userList)
{
	...
	string folder_path;
	folder_path.append("users/");
	folder_path.append(usr.username.c_str());
	if (mark_directory(folder_path.c_str()))
	{
	...
	string fa_path = folder_path + "/fa.txt";
	if (!createFile(fa_path.c_str()))
	{
		printf("create fa file error\n");
		exit(0);
	}
	folder_path.append("/back_up");
	if (mark_directory(folder_path.c_str()))
	{
		printf("create back_up folder error\n");
		exit(0);
	}
	//save account
	string s;
...
int getGC_response(char **res, int &n, char mess[], int mess_len, ClientInfo &clientInfo)
{
	...
	string folder_path;
	folder_path.append("users/");
	folder_path.append(clientInfo.username);
	folder_path.append("/");
	printf("folder_path %s\n", folder_path.c_str());
	string file_names;
	int len = getFileNames(folder_path, file_names);
	printf("len %d\n", len);
	int begin = 0;
	int buff_size = 2;
	while (begin < len)
...
int getAA_response(char **res, int &n, char mess[], int mess_len, ClientInfo &clientInfo)
{
	...
	int rt = getCategoryAndAddress(mess, mess_len, category, address);
	if (rt != -1)
	{
		res[n++] = getNoticeMess(rt);
		return -1;
	}
	if (!category.compare("fa"))
	{
	...
	fa_path.append("fa.txt");
	file_path.append(category.c_str());
...
int getSF_response(char **res, int &n, char mess[], int mess_len, ClientInfo &clientInfo)
{
	...
	string username, category, address;
	int rt = getUsernameAndCategoryAndAddress(mess, mess_len, username, category, address);
	if (rt != -1)
	{
		res[n++] = getNoticeMess(rt);
		return -1;
...
int getGA_response(char **res, int &n, char mess[], int mess_len, ClientInfo &clientInfo)
{
	...
	string category_path = "users/";
	category_path.append(clientInfo.username);
	category_path.push_back('/');
	category_path.append(category.c_str());
	category_path.append(".txt");
	printf("%s file path\n", category_path.c_str());
	//read file category_name.txt
	int cgBytes = getFileSize(category_path.c_str());
	if (cgBytes < 0)
	{
		res[n++] = getNoticeMess(11);
		return -1;
	...
	{
		char *buff = new char[BUFFSIZE];
		int length = readFileBlock(category_path.c_str(), beginPos, buff + 3, buff_size);
		if (length <= 0)
		{
			delete (buff);
			break;
...
int getBK_response(char **res, int &n, char mess[], int mess_len, ClientInfo &clientInfo)
{
	...
	backupFilePath.append(category.c_str());
	backupFilePath.append(".txt");
	printf("%s is backup file path\n", backupFilePath.c_str());

	int rt = copyFile(category_path.c_str(), backupFilePath.c_str());
	if (rt == -1)
	{
		res[n++] = getNoticeMess(11);
		return -1;
...
int getRS_response(char **res, int &n, char mess[], int mess_len, ClientInfo &clientInfo)
{
	...
	backupFilePath.append(".txt");
	printf("%s is backup file path\n", backupFilePath.c_str());
	cgBytes = getFileSize(category_path.c_str());
	if (cgBytes < 0)
	{
	...
	int rt = copyFile(backupFilePath.c_str(), category_path.c_str());
	if (rt == -1)
	{
		res[n++] = getNoticeMess(11);
		return -1;
